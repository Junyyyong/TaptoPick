...
let unitLayout;
for(const [mode,taps] of [['unit',4],['montage',1],['memory',4]]){
...
if(mode!=='memory'){
const layout={preview:await page.locator('.visual-practice-preview').boundingBox(),board:await page.locator('.practice-board').boundingBox()};
if(mode==='unit'){unitLayout=layout;assert(layout.board.y-layout.preview.y-layout.preview.height>=24);}
assert.equal(await page.locator('.practice-tile').count(),mode==='unit'?9:4);
if(mode==='unit'){
const tile=await page.locator('.practice-tile').first().boundingBox();
const oldWidth=size.height>700?Math.min((size.width-48)*.7,238,size.height*.273):Math.min((size.width-48)*.7,size.height*.238);
assert(Math.abs(tile.width-(oldWidth-4.9)/2)<1);
await page.locator('[data-target="false"]').first().click();assert.equal(await page.locator('.practice-status').textContent(),'Not Tepee');assert.equal(await page.locator('.practice-piece-color.is-revealed').count(),0);
}
}
...
}else{const preview=await page.locator('.visual-practice-preview').boundingBox();assert(preview.height>=size.height*.25);}
...
assert.equal(await page.locator('.practice-tile.is-guided').count(),1);if(mode!=='unit')assert.equal(await page.locator('.practice-tile:enabled').count(),1);
await page.locator('.practice-tile.is-guided').click({force:true});}
...
await page.locator('[data-next]').click({force:true});}
