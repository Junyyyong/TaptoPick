...
## 후속 수정 — 첫 게임 아이콘을 3×3 격자로

사용자 요청으로 2×2와 분리된 조각 모양을 하나의 3×3 격자 박스로 바꿨다. 흰색 선·둥근 모서리·선 두께 2.4·아이콘 영역·파란 판은 그대로 유지했다. 다른 게임 아이콘과 게임 규칙은 변경하지 않았다. 위의 `intro-unit.png`는 변경 전 기록으로 남긴다.

![3×3 격자로 바꾼 첫 게임 아이콘](intro-unit-grid.png)

`check-grid-icon.cjs`로 SVG의 격자 구성·판 크기·START 후 49칸 진입을 확인했다. 캡처 조건은 위와 동일하며 255개 테스트와 빌드도 통과했다.
