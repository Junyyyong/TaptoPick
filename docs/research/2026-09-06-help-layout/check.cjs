...
for(const [mode,taps] of [['unit',3],['montage',1],['memory',4]]){
...
const preview=await page.locator('.visual-practice-preview').boundingBox();assert(preview.height>=size.height*.24);
...
await page.locator('#btn-title-tutorial').click();await page.locator('[data-practice-mode="memory"]').click();await page.locator('.practice-tile.is-guided').click();await page.locator('[data-retry]').click();assert.equal(await page.locator('.practice-tile.is-revealed').count(),0);
