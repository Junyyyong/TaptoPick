...
async function testPolicy(policy) {
  ...
  try {
    for (const viewport of [{width: 390, height: 844}, {width: 375, height: 667}, {width: 320, height: 568}]) {
      ...
      page.on('request', request => { if (/pick-(garden|lobby).*\.mp3/.test(request.url())) requests.push(request.url()); });
          ...
          createBufferSource() {
            ...
            source.start = (...args) => {
              if (source.loop) {
                ...
                record.track = Math.abs(record.duration - 40) < .01 ? 'menu' : Math.abs(record.duration - 38.4) < .01 ? 'game' : 'unknown';
      ...
      assert.equal(requests.filter(r => r.includes('pick-lobby')).length, 1);
...
(async () => {
  ...
  console.log('Autoplay allowed/blocked policies passed on all three mobile viewports.');
